...
-- =====================================================
-- FASE 2: Función compartida de resolución de theme
-- =====================================================

-- 1. Función interna: resuelve el theme dado un product_id y merchant_id
--    Jerarquía: product template > merchant default template > hardcoded default
...
DECLARE
  ...
  v_hide_fields jsonb;
BEGIN
  -- Get product (for checkout_template_id)
  SELECT * INTO v_product FROM public.products WHERE id = p_product_id;
  
  -- Get merchant (for business_name fallback)
  SELECT * INTO v_merchant FROM public.merchants WHERE id = p_merchant_id;

  -- Resolve template: product-specific > merchant default > none
...
-- 2. RPC pública: configuración completa para product landing (/p/:slug)
...
BEGIN
  ...
  -- Resolve theme using shared function
...
-- 3. Actualizar get_checkout_config para usar resolve_checkout_theme
